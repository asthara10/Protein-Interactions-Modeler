#!/usr/bin/perl
#Baldo Oliva
#14/02/18  César J. Enríquez-Rodriguez : double aminoacid w/multiple conformations in Fasta file [fixed], alert info and improve of PDB file.
...
$lode="ACDEFGHIKLMNPQRSTVWYCGTA";
 ...
 for $k (0..$#out )
 {
   ...
   for $i (0..$#prot)
   {
     ...
     $altloc= $prot[$i]->{altloc};
     $letter="X";
	 if ((length $altloc)!= 0){
	 	if ($altloc eq A){
            if($prot[$i]->{atom} eq " CA" || $prot[$i]->{atom} eq " P"){print ("Info: Residue $resnum ($res) has multiple conformations\n")}
		}
		else{
			next;
		}
...
format PDB_FORMAT=
@<<<<<@>>>> @<<<@@>> @@>>>    @>>>.@<<@>>>.@<<@>>>.@<<@>>.@<@>>.@<
$typew,$atnumw,$atomw,$altlocw,$resw,$chainw,$resnumw,$x1,$x2,$y1,$y2,$z1,$z2,$o1,$o2,$b1,$b2
.
