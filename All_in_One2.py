...
def Superimpose_chains(temp_obj, PDB_bychain_objects):

	ref_model = temp_obj[0]
	#ref_chains = Selection.unfold_entities(ref_model, 'C')

	for sample_structure in PDB_bychain_objects:
		atoms_to_be_aligned = range(1, len(sample_structure.get_sequence()))
		...
		#sample_atoms = Selection.unfold_entities(sample_model, 'A')




	"""for alt_model in temp_obj:
